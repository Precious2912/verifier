...
public static class Queries
{
        ...
        public const string GetVerificationCheckpoint = """
                SELECT last_created_at AS LastCreatedAt, last_id AS LastId
                FROM event_store.verification_checkpoint 
                WHERE id = 1;
                """;
        ...
        public const string GetTransactions = """
                SELECT "Id" AS Id, "Reference" AS Reference, "Type" AS Type, "DebitAccount" AS DebitAccount, 
                       "CreditAccount" AS CreditAccount, "Amount" AS Amount, "CreatedAt" AS CreatedAt
                FROM "Transactions";
                """;
        ...
        public const string GetTransactionsInSlice = """
                SELECT "Reference" AS Reference, "Type" AS Type, "DebitAccount" AS DebitAccount, 
                       "CreditAccount" AS CreditAccount, "Amount" AS Amount, "CreatedAt" AS CreatedAt, "Id" AS Id
                FROM "Transactions"
                WHERE ("CreatedAt", "Id") >  (@fromCreatedAt, @fromId) AND ("CreatedAt", "Id") <= (@toCreatedAt, @toId)
        ...
        public const string GetEventsForReferences = """
                SELECT stream_id AS StreamId, type AS Type,
                        ...
                        (data ->> 'OccurredAt')::timestamptz AS OccurredAt
                FROM event_store.mt_events
                WHERE type IN ('account_debited', 'account_credited') AND data ->> 'Reference' = ANY(@references)
                ORDER BY seq_id;
                """;
        ...
        public const string InsertResults = """
                INSERT INTO evaluation.detection_results
                        (id, scenario, scale, fault_type, target_ref, target_account, numeric_caught, record_caught,
                        snapshot_caught, numeric_fps, record_fps, numeric_load_ms, numeric_compute_ms, record_load_ms, 
                        record_compute_ms, snapshot_load_ms, snapshot_compute_ms, fault_count, scored_at)
                VALUES
                        (@Id, @Scenario, @Scale, @FaultType, @TargetRef, @TargetAccount, @NumericCaught, @RecordCaught, 
                        @SnapshotCaught, @NumericFPs, @RecordFPs, @NumericLoadMs, @NumericComputeMs, @RecordLoadMs, 
                        @RecordComputeMs, @SnapshotLoadMs, @SnapshotComputeMs, @FaultCount, @At);
                """;
