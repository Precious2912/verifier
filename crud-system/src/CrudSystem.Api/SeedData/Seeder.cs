...
namespace CrudSystem.Api.SeedData;

public static class Seeder
{
    // Handles CSV parsing and seeding
    public static async Task SeedAsync(AppDbContext db, string csvPath, int count)
    {
        var transfers = File.ReadLines(csvPath).Skip(1).Take(count)
            .Select(l => l.Split(','))
    ...
    //Handles seeding operation
    public static async Task RunAsync(IServiceProvider services, IConfiguration config, string contentRoot)
    {
        var count = config.GetValue<int?>("DataSetConfig:RecordCount")
            ?? throw new InvalidOperationException("DataSetConfig:RecordCount is missing.");
        if (count <= 0)
            throw new InvalidOperationException($"RecordCount must be positive, got {count}.");

        var configuredPath = config.GetValue<string>("DataSetConfig:CSVPath")
            ?? throw new InvalidOperationException("DataSetConfig:CSVPath is missing.");

        var csvPath = Path.IsPathRooted(configuredPath)
            ? configuredPath
        ...
        await SeedAsync(db, csvPath, count);
    }
}